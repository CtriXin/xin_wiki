...
async function bootstrap () {
  ...
  const ui = {
    ...
    learningTarget: document.getElementById('learningTarget'),
    learningLevel: document.getElementById('learningLevel'),
    learningStreak: document.getElementById('learningStreak'),
    learningAccuracy: document.getElementById('learningAccuracy'),
    learningQuestion: document.getElementById('learningQuestion'),
  ...
  const learningSubmit = document.getElementById('learningSubmit');
  const learningRetarget = document.getElementById('learningRetarget');
  ...
  function renderLearningButton (enabled) {
    if (!learningBtn) return;
    if (document.body.classList.contains('compact-ui')) {
      learningBtn.textContent = enabled ? '🧠' : '📗';
      learningBtn.title = enabled ? '学习模式已开启' : '学习模式';
    } else {
      learningBtn.textContent = enabled ? '学习中' : '学习模式';
    }
  }
  ...
  learningBtn?.addEventListener('click', () => {
    const enabled = game.toggleLearningChallenge();
    renderLearningButton(enabled);
  });
  ...
  learningSubmit?.addEventListener('click', () => {
    const input = learningInput?.value || '';
    game.submitLearningPracticeAnswer(input);
    if (learningInput) learningInput.value = '';
  });
  learningInput?.addEventListener('keydown', (evt) => {
    if (evt.key !== 'Enter') return;
    const input = learningInput.value || '';
    game.submitLearningPracticeAnswer(input);
    learningInput.value = '';
  });
  learningRetarget?.addEventListener('click', () => {
    game.rollLearningTarget();
  });
  ...
  introStart?.addEventListener('click', () => {
    ...
    if (learningBtn) {
      renderLearningButton(game.isLearningChallenge());
    }
    if (styleBtn) {
      renderStyleButton(game.game.visualStyle);
    }
