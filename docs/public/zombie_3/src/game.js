...
export class ZombieGame {
  ...
  toggleLearningChallenge () {
    ...
    if (this.game.learning.enabled) this.game.learning.targetAnswer = this.rollLearningTarget();
  ...
  rollLearningTarget () {
    return 2 + Math.floor(Math.random() * 18);
  }
  ...
  cycleMiningMap () {
    ...
    for (let i = 0; i < 4; i++) this.spawnOreNode();
  ...
  rollOreType () {
    const r = Math.random();
    if (r < 0.5) return { type: 'stone', hp: 24, color: '#95a5a6', reward: { scrap: 2, gems: 4, core: 0 }, xp: 8 };
    if (r < 0.78) return { type: 'copper', hp: 32, color: '#c97a40', reward: { scrap: 4, gems: 7, core: 0 }, xp: 12 };
    if (r < 0.93) return { type: 'crystal', hp: 40, color: '#74b9ff', reward: { scrap: 2, gems: 16, core: 1 }, xp: 18 };
  ...
  spawnOreNode () {
    ...
    const x = this.game.world.minX + 26 + Math.random() * (this.game.world.maxX - this.game.world.minX - 52);
    const y = this.game.world.minY + 44 + Math.random() * (this.game.world.maxY - this.game.world.minY - 80);
  ...
  updateMiningField (ts = performance.now()) {
    ...
    const depthMul = 1 + Math.min(1.6, this.game.mining.depth * 0.08);
    const targetCount = Math.min(12, 6 + Math.floor(this.game.mining.depth / 3));
    const interval = Math.max(680, 1500 - this.game.mining.depth * 32);
    if (this.game.oreNodes.length < targetCount && ts - this.game.mining.lastSpawnAt > interval) {
      this.spawnOreNode();
      this.game.mining.lastSpawnAt = ts;
    }
    this.game.oreNodes = this.game.oreNodes.filter((n) => n.hp > 0);
    this.game.oreNodes.forEach((n) => {
      n.pulse += 0.03;
      n.maxHp = n.baseHp * depthMul;
  ...
  createMathProblem () {
    if (this.game.learning.enabled) {
      ...
      const preferTarget = Math.random() < 0.42;
      if (preferTarget) {
        const type = Math.random();
        if (type < 0.4) {
          const a = 1 + Math.floor(Math.random() * (target - 1 || 1));
          const b = target - a;
          return { question: `${a}+${b}`, answer: String(target), value: target };
        }
        if (type < 0.8) {
          const a = target + (1 + Math.floor(Math.random() * 9));
          const b = a - target;
          return { question: `${a}-${b}`, answer: String(target), value: target };
        }
        const factors = [];
        for (let i = 1; i <= 9; i++) {
          if (target % i === 0 && target / i <= 9) factors.push([i, target / i]);
        }
        if (factors.length) {
          const pair = factors[Math.floor(Math.random() * factors.length)];
          return { question: `${pair[0]}×${pair[1]}`, answer: String(target), value: target };
        }
        return { question: `${target}+0`, answer: String(target), value: target };
      }
      // 非目标怪，确保答案不等于目标
      const wrong = this.rollLearningTarget();
      const val = wrong === target ? ((wrong % 19) + 1) : wrong;
      const a = 1 + Math.floor(Math.random() * (val - 1 || 1));
      const b = val - a;
  ...
  refreshSketchHint () {
    ...
    this.sketchHint.innerHTML = `层:${layerName} · ${cls}<br>装备:${gear.icon}${gear.name}(${rarityName})${gearLimit}<br>${bonusText} · 技能T${stage}:${skillName}(${cooldownText})<br>${classExtra} · ${tacticText}<br>${mineText}`;
  }
  ...
  updateUI () {
    ...
    if (this.ui.mineStamina) this.ui.mineStamina.textContent = `${Math.floor(this.game.mining.stamina)}/${this.game.mining.maxStamina}`;
  ...
  forgeCraftGrid (cells) {
    ...
    const quality = count.core * 1.2 + count.gem * 0.65 + count.scrap * 0.3;
    let failRate = 0.42 - quality * 0.1 - Math.min(0.12, (repeat - 1) * 0.02);
    failRate = this.clamp(failRate, 0.06, 0.55);

    if (Math.random() < failRate) {
      // 失败有小概率返还部分材料，避免挫败感。
      if (Math.random() < 0.35) this.game.materials.scrap += 1;
      if (Math.random() < 0.2) this.game.gems += 1;
    ...
    // 同比例配方会有多种可能结果。
    let pool = ['upgrade'];
    if (count.core >= 2) pool = ['unlock', 'synthesize', 'upgrade'];
    else if (count.core >= 1 && count.scrap >= 2) pool = ['unlock', 'upgrade', 'bonus'];
    else if (count.gem >= 2) pool = ['upgrade', 'repair', 'bonus'];
    else if (count.scrap >= 3) pool = ['unlock', 'bonus', 'repair'];

    const roll = Math.random();
    const outcome = pool[Math.floor(roll * pool.length)] || 'upgrade';
    ...
    if (outcome === 'unlock') ok = this.forgeUnlockNextGear({ skipCost: true });
      ...
      this.game.gems += 30 + count.gem * 12;
      this.game.materials.scrap += 2 + count.scrap;
      if (count.core > 0) this.game.materials.core += 1;
  ...
  handlePointer (e) {
    ...
    if (this.game.currentClass === 'miner' && oreTarget) {
      const hit = this.mineOreNode(oreTarget, 'tap');
      if (hit) {
        this.player.attackCooldown = Math.max(this.player.attackCooldown, 8);
      }
      return;
    }
    ...
    if (this.game.learning.enabled) {
      if (tapTarget) {
        ...
        if (thisVal === targetVal) {
          this.damageEnemy(tapTarget, Math.max(120, tapTarget.maxHp * 3), '#55efc4');
          this.createFloatingText(tapTarget.x, tapTarget.y - 74, '命中目标', '#55efc4', 13);
        } else {
          this.player.hp = Math.max(0, this.player.hp - 12);
          this.createFloatingText(tapTarget.x, tapTarget.y - 74, '点错 -12HP', '#ff7675', 13);
          if (this.player.hp <= 0) this.gameOver();
        }
        return;
      }
  ...
  start () {
    ...
    this.game.mining.quest = this.buildMiningQuest();
    if (this.isMiningMode()) {
      for (let i = 0; i < 3; i++) this.spawnOreNode();
    }
    this.updateUI();
